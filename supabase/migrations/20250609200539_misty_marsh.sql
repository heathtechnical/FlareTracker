...
-- Medications table with enhanced fields
CREATE TABLE IF NOT EXISTS medications (
  ...
  frequency_type text DEFAULT 'as-needed' CHECK (frequency_type IN ('scheduled', 'as-needed', 'limited-course')),
  notes text,
  active boolean DEFAULT true,
  condition_ids text[] DEFAULT '{}',
  max_usage_days integer,
  start_date timestamptz,
  warning_threshold integer,
  category text DEFAULT 'other' CHECK (category IN ('topical-steroid', 'antibiotic', 'moisturizer', 'other')),
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
